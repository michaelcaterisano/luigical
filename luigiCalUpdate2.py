...
def update_calendar(events_to_add=[], events_to_delete=[]):
    ...
    while True:
        ...
        events = service.events().list(calendarId='gcalv2@iceorg.org', pageToken=page_token, singleEvents=True).execute()
    ...
    for event in events_to_add:
        ...
        if key not in eventIdTable:
            event = service.events().insert(calendarId='gcalv2@iceorg.org', body=event).execute()
    ...
    for event in events_to_delete:
        ...
        if key not in eventIdTable:
            ...
            break
        service.events().delete(calendarId='gcalv2@iceorg.org', eventId=eventIdTable[key]).execute()
